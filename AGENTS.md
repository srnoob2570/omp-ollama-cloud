...
oh-my-pi plugin registering an **Ollama Cloud provider** for omp: model catalog + official USD pricing from the `srnoob2570/ollama-cloud-catalog` artifact, a live streaming-stats widget, a workaround for omp's $0-cost `ollama-chat` adapter bug, and an npm self-update probe. Runs inside the Bun-based omp runtime — not standalone.
...
catalog artifact (2 CDN mirrors, mirror-race, 5s timeout)
  ...
  → index.ts    pi.registerProvider("ollama-cloud", { fetchDynamicModels })
...
- `src/` — all source, flat, ~8 modules + 5 colocated test files. No subdirectories.
...
- **Config via env knobs, read once at factory time** (omp passes no factory options): `OMP_OLLAMA_CLOUD_PRICING`, `OMP_OLLAMA_CLOUD_COST_FIX`, `OMP_OLLAMA_CLOUD_STATS` — all default on; `off`/`0`/`false`/`no` disable.
