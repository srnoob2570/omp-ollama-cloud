// Catalog loader for the published ollama-cloud-catalog artifact. Direct port
// of opencode-ollama-cloud/plugin/catalog.ts minus the opencode-specific
// pieces (ProviderConfig/npm plumbing stays validated but unused by omp; the
// cache path moved to ~/.cache/omp-ollama-cloud/).
...
export interface PricingRate {
  input: number;
  output: number;
  /** Rate card's "Cached input" column — feeds omp's cost.cacheRead. */
  cachedInput?: number;
}
...
const CACHE_DIR = join(homedir(), ".cache", "omp-ollama-cloud");
...
export function isCatalog(value: unknown): value is ArtifactDoc {
  ...
  return Object.values(models).every((raw) => {
    ...
    return (
      ...
      // consumed downstream as the model's release date, a required string
      ...
      (x === undefined ||
        ((x.quantization === undefined ||
          ...
          // garbage effort keys never reach the picker or a 400 upstream
